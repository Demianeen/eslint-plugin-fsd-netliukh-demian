...
module.exports = {
  ...
  create(context) {
    ...
    const shouldBeRelative = (from, to) => {
      ...
      const normalizedPath = path.normalize(from) // .../src/entities/User/ui/UserCard
      const projectFrom = normalizedPath.split('src')[1] // ['...', /entities/User/ui/UserCard]
      const fromArray = projectFrom.split(path.sep) // ['', 'entities', ...]
